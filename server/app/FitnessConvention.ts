...
export const evaluate = (bots: Bot[]) => {
  ...
  for(let j = 0; j < bots.length; j++){
    ...
    for(let i = 0; i<bots[j].melody.notes.length; i++){
      octave.push(bots[j].melody.notes[i].getOctave())
      if(i==0){
        min_octave = octave[i]
          max_octave = octave[i]
      }else{
    ...
    let step_fitness = 0
    let note_diff
    for(let i = 0; i<bots[j].melody.notes.length-1; i++){
      note_diff.push(bots[j].melody.notes[i].compare(bots[j].melody.notes[i+1]))
      if(Math.abs(note_diff[i]) <= 1){
        step_fitness+=1
      }
    }
    if(step_fitness > bots[j].melody.notes.length*0.75){
      step_fitness = 1
    }else{
      step_fitness = step_fitness/bots[j].melody.notes.length
    }
    ...
    for(let i = 0; i<bots[j].melody.notes.length; i++){
      if(mill_whoop[i] == bots[j].melody.notes[i].getKey()){
        if (i > 0){
          if((octave[i] == octave[i-1]) && (bots[j].melody.notes[i-1].getKey() == mill_whoop[i-1])){
            whoop_fitness+=1
          }
        }else{
          whoop_fitness+=1
    ...
    whoop_fitness = (2**whoop_fitness)/(2**bots[j].melody.notes.length)
