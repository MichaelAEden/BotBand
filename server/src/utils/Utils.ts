...
export const parseBotsFromReq = (req) => {
  return req.body.bots.map(
    (bot) =>
      new Bot(
        bot.metric, 
        new Melody(bot.melody.notes.map((note) => Note.fromString(note.note))), 
        bot.playCount
      )
  );
};
...
export const selectRandomWeightedNoReplacement = (items, weights, n) => {
